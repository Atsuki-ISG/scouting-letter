...
テンプレートの `type` 列と求人の `employment_type` は整合させること。
拡張の雇用形態ドロップダウン・求人自動選択・ミスマッチ警告はこの値で制御される。

| テンプレート type | 求人 employment_type | 拡張ドロップダウン表示 |
|---|---|---|
| `正社員_初回` / `正社員_再送` | `正職員` or `正社員` | 正社員 |
| `パート_初回` / `パート_再送` | `パート` | パート |
| `契約_初回` / `契約_再送` | `契約社員` or `契約職員` | 契約社員 |

- 1種類の雇用形態しかない会社でも、他の希望の候補者にフォールバックで生成される（警告つき）
- 新しい雇用形態を追加する場合は、サーバーの `_EMPLOYMENT_TYPE_DISPLAY_NAMES` にも登録が必要
