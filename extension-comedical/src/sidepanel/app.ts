/**
 * Comedical sidepanel app. 極小UI。履歴表示・CSV出力・使い方ガイド。
 *
 * 「送信する」操作はここには無い。送信は候補者一覧ページ上のモーダル
 * でオペレータがコメディカルの送信ボタンを押す（content script は
 * 自動フィルするだけ）。その方が誤送信事故が起きない。
 */
...
async function onCsvExport() {
  ...
  a.href = url;
  a.download = `${BUILD_CONFIG.companyId}-comedical-history-${date}.csv`;
  a.click();
