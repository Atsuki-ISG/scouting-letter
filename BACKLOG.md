...
- [ ] **他媒体向けChrome拡張の作成（ウェルミー / コメディカル.com 等）** — 現状はジョブメドレー専用。医療・介護系の他媒体にも送信アシスト・プロフィール抽出・API生成を展開する。必要な作業:
  - 各媒体のスカウト画面・候補者一覧のDOM調査（セレクタ・非同期読み込みタイミング）
  - プロフィール抽出ロジックの媒体別分岐（`extension/src/content/extractors/` 配下を媒体別ファイルに分割）
  - 会社自動判定・バリデーション・送信アシストの媒体別対応
  - 送信データシートの媒体列を活用した返信率比較
