...
- [ ] **AI生成の範囲を拡大する** — 現状AI生成は「personalize部分（テンプレ本文の `{ここに生成した文章を挿入}` に入る2-3文）」だけ。テンプレ本文は固定。AIが生成する範囲を広げることで質を上げる余地があるか探る。
  - **まずは実現方法の調査から**:
  ...
  - **候補ソース**: 既存の personalized_scout パイプライン (L2/L3)、`improve_template` のテンプレ再生成、profile.md の差別化要素を動的に織り込む等
