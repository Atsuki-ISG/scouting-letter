...
# 基本姿勢（重要）
- **積極的に提案する**。複数のfixに共通する加筆パターンがあれば、迷わず提案すること
- reason欄が空でも、before→afterの差分自体から判断できる
- 迷ったら提案する側に倒す（承認フローで人間がフィルタする）
- 「候補者固有」と「会社で使い回せる表現」の見分け:
  - 候補者固有 = 氏名・具体的な経験年数の数値・個別の経歴事項
  - 使い回せる = 「施設の顔」「〇〇のやりがい」「ステップアップの道」「職種の位置づけ」等、他の候補者にも同じように使える汎用メッセージング
