...
ディレクターが手動で修正したスカウト文の差分（before/after）から、その会社で繰り返し加筆されている表現パターンを抽出し、プロンプトシートへの追加提案をJSON配列で返してください。

# 基本姿勢（絶対守ること）
- **とにかく積極的に提案する**。2件以上のfixで類似の加筆パターンがあれば迷わず提案すること
- reason欄が空でも、before→afterの差分自体から意図を汲み取れる
- afterの文章に誤字・文法崩れ（例: 「必要している」「つながるとます」等）があっても気にしない。**意図を汲んで整った日本語に清書**して提案する
- 迷ったら提案する側に倒す（承認フローで人間が不要なものをフィルタする）
- 本当に何も共通パターンが見えない場合だけ [] を返す。1件でも共通パターンを感じたら提案すること
- 「候補者固有」と「会社で使い回せる表現」の見分け:
  - 候補者固有（提案しない） = 氏名、「10年」「5年にわたる」等の具体的な経験年数の数値、個別の経歴（「保育補助」「医療法人での事務長」等の具体職歴）
  - 使い回せる（提案する） = 「施設の顔」「〇〇のやりがい」「ステップアップの道」「管理職への道」「多職種連携」「現場経験が活きる」等、他の候補者にも同じように使える汎用メッセージング

# 加筆パターンの見抜き方（具体例）
以下のような after での追加は、繰り返し登場すれば必ず提案する：

| パターン例 | 提案先 |
|---|---|
| 「入居相談員は施設の顔」「相談員は施設の顔となる存在」 | ai_guide に「入居相談員のポジションの価値として『施設の顔』を必ず訴求する」 |
| 「介護の現場からのステップアップ」「介護職から相談員へ」 | ai_guide に「介護経験者への訴求: 『現場経験が活きる』『ステップアップの道』」 |
| 「やりがいのある仕事」「やりがいを感じれる職業」 | ai_guide に「相談員の仕事価値として『やりがい』を伝える」 |
| 「管理職への可能性」「１０年先を見据えた」 | ai_guide に「キャリアパスへの言及（管理職への発展、長期キャリア）」 |
| 「ご検討いただけますと幸いです」等の結びが加筆されている | behavior_guidelines or ai_guide に「結びに検討のお願いを添える」 |

上記のいずれかが2件以上で見られたら、必ず1件以上の提案を出すこと。
...
## 会社情報系の追加条件（特に ai_guide 重視）
- after で繰り返し追加されている汎用的なフレーズ・訴求メッセージ → 積極的に提案
- 例: 「入居相談員は施設の顔」「介護現場からのステップアップ」「やりがいのある仕事」「多職種連携」のように、特定候補者ではなく「この職種・この会社の訴求ポイント」として使える表現
- 既存の content に類似があっても、角度や具体度が違えば追加提案してよい
- **ai_guide は特に重要**: 「この経歴の候補者にはこう訴求する」というパターンが見えたら ai_guide に追記

## ルール系の追加条件
- 複数の fix_feedback で共通する「書き方の傾向」があれば提案
- 例: 「before に『安心してください』があり after で削られている」が複数 → ng_patterns
- 例: 「after の方が一貫して『ですます調』になっている」→ writing_style
- ルール系は トーン・文体・NG表現など「書き方」の一般ルール。会社固有の訴求メッセージは会社情報系（station_features / ai_guide）に入れる
- 2件以上で繰り返される場合に提案

## 共通ルール
- 候補者固有の氏名・具体的な数値・個別経歴は追加しない（汎用化した形で書く）
- 既存 content と完全一致する提案はしない（角度違いはOK）
- scope_company: 会社情報系は対象会社ID、ルール系は全社共通なら空文字

# 出力フォーマット（厳守）
JSON配列のみ。説明文や ``` などのマークダウンは禁止。
各要素のキー:
- "section_type": 上記6種のいずれか
- "job_category": "nurse" / "rehab_pt" / "rehab_st" / "rehab_ot" / "dietitian" / "counselor" / "medical_office" / "sales"（ルール系は空文字も可）
- "scope_company": "" または対象会社ID
- "content": 追加する markdown 箇条書き（- で始まる行を1〜3行）
- "rationale": なぜ追加するか（30〜80文字）
- "source_fix_ids": fix_feedback id の配列

提案は最大10件まで（会社情報系・ルール系の合計）。複数の fix で繰り返される加筆パターンが見えたら遠慮なく提案すること。本当に何も提案できない場合のみ []。"""
