...
**トーン: 会社側が「力を借りたい／必要としている」姿勢で書く**

候補者が「選ばれる側」ではなく「会社が力を必要としている側」という関係を前提にする。**上から目線になる表現を避ける**:

- ✕「活躍いただけます」「力になっていただけます」（会社が舞台を与える感／上から目線）
- ✕「期待に応えていただけます」「貢献いただけます」（業績評価の目線）
- ◎「お力をお借りしたいと考えています」（会社側が依頼する姿勢）
- ◎「ご経験が必要な場面です」「ご知見を必要としている局面です」（必要性の率直な表明）
- ◎「〜の役割を担っていただけると心強く思います」（候補者への依頼＋会社側の心情）
- ◎「〜の場面で、ご経験をお借りしたいポジションです」（職務提示＋依頼）

**推測の断定を避ける（捏造防止の一環）**

会社情報・プロンプトセクション・求人情報に**明記されていない** つながりを「〜可能です」「〜できます」と言い切らない。一般論で想定される接続（例：地域包括ケア病棟 = 退院支援・多機関連携、訪問看護 = ターミナルケア）は、**明記がない限り控えめな可能性表現**にする:
...
profile.md・求人情報・プロンプトセクションにない項目は、可能なら**言及自体を避けて**別の明記された要素に切り替える方が安全。
