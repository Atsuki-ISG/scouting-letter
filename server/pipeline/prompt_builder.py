...
def build_system_prompt(
    ...
    parts.append(
        ...
        "\n"
        "### 厳禁: 思考過程・メタ情報の出力\n"
        "以下のような出力は本文に絶対に含めないでください（思考の中だけで完結させ、最終出力には一切残さない）:\n"
        "- 「Draft 1:」「Draft 2:」「Draft 2 (Applying ...):」などの下書きラベル\n"
        "- 「Character Count Check」「Character count:」「(119 characters)」「(39 chars)」などの文字数注記\n"
        "- 「Final Text:」「Final Version:」「Final Output:」「最終版」などの最終確認ラベル\n"
        "- 「Wait,」「Actually,」「Hmm,」「Let me ...」などの自己対話・自己訂正コメント\n"
        "- 「118文字。」「これが一番事実に基づいている」などの自己検証コメント\n"
        "- マークダウン記法の見出し・コードブロック（```）\n"
        "出力は、テンプレートに差し込めば即送信できる、完成したパーソナライズ本文のみとしてください。"
    )
